...
## 5. teacher_id（Context Key）用什么？——需要你拍板

CONTEXT KEY 必须是"每个老师稳定、且互不相同"的字符串。选一个：
- **平台的登录用户ID / session ID**（最稳，若平台能取到，首选）
- **老师的邮箱**
- **让老师在 query 里带一个自己的编号**（最简单，但靠老师自觉）

师兄 demo 里是用 Text Merge 手填 `teacher_1`/`teacher_2` 模拟的——真实环境要换成
上面某个稳定来源。**这一步定不好，多个老师的 KB 会互相串或读不到。**
